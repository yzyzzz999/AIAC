...
def run_smoke_test():
    ...
    results.append(check(
        prov.get("calibration_status") == "spring_summer_v8_l1_candidate_not_vehicle_calibration",
        "calibration_status 正确"
    ))
    results.append(check(
        prov.get("chtd_param_mode") in ("stage2_v4_calibrated", "safe_preview"),
        "chtd_param_mode is v4-calibrated"
    ))
